...
int main(int argc, char **argv){
    ...
    Imlib_Image buffer;
    char title[255], *prefix=getenv("PREFIX"); if (!prefix) prefix=&("");
    ...
    joincmdlinetext();
    printf("> prog %s %d T:%s<%d %d\n",program_name,multiple_instances,prefix,windowed,desktop_mode);
    // If an instance is already running, quit
    if (!multiple_instances)
    {
        int oldmask = umask(0);
        lock = open(concat(prefix,"/tmp/xlunch.lock"), O_CREAT | O_RDWR, 0666);
        umask(oldmask);
    ...
    imlib_add_path_to_font_path(concat(prefix,"/usr/local/share/fonts"));
    imlib_add_path_to_font_path(concat(prefix,"/usr/share/fonts/truetype"));
    imlib_add_path_to_font_path(concat(prefix,"/usr/share/fonts/truetype/dejavu"));
    imlib_add_path_to_font_path(concat(prefix,"/usr/share/fonts/TTF"));
